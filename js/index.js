var app = {
    ...
    getAppDb: function() {
        ...
        return openDatabase('dbSaans', localStorage.getItem('dbLocalVersion'), 'dbSaans', (5 * 1022 * 1022));
    },
    getImgDb: function() {
        if(localStorage.getItem('imgDbLocalVersion') == null) {
            localStorage.setItem('imgDbLocalVersion','-1');
        }
        return openDatabase('imgDbSaans', localStorage.getItem('imgDbLocalVersion'), 'imgDbSaans', (5 * 1022 * 1022));
    },
    initialize: function() {
    ...
    doOnlineTasks: function() {
        var urlData = 'http://saans.incorelabs.com/db_version.php';
    ...
    checkWithLocalDB: function(json) {
        if (localStorage.getItem("dbLocalVersion") != json[0][0]) {
            ...
            $.getJSON('http://saans.incorelabs.com/users.php', function(userData) {
                app.createTable(userData,"users",0);
            });
            $.getJSON('http://saans.incorelabs.com/male.php', function(maleData) {
                app.createTable(maleData,"male",1);
            });
            $.getJSON('http://saans.incorelabs.com/female.php', function(femaleData) {
                app.createTable(femaleData,"female",2);
            });
            $.getJSON('http://saans.incorelabs.com/common.php', function(commonData) {
                app.createTable(commonData,"common",3);
            });
            $.getJSON('http://saans.incorelabs.com/kids.php', function(kidsData) {
                app.createTable(kidsData,"kids",4);
            });
            $.getJSON('http://saans.incorelabs.com/directors.php', function(directorsData) {
                app.createTable(directorsData,"directors",5);
            });
            $.getJSON('http://saans.incorelabs.com/events.php', function(eventsData) {
    ...
    getImageAssets: function () {
        var urlImages = 'http://saans.incorelabs.com/images/file-list.php?key=kamlesh';
    ...
    getSecondAd: function() {
        var urlImages = 'http://saans.incorelabs.com/adTwo/file-list.php?key=kamlesh';
        var dirReference = app.getDirectoryReference();
        dirReference.done(function(imgDir) {
            app.imgDir = imgDir;
